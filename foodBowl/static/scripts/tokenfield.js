$(document).ready(function () {
  ...
  /* Get match recipes by list of user ingredient input via AJAX. List of categories is used to filter in views.py */
  function filter(list_recipe) {
    $('#recipe').empty();
    $.ajax({
      type: 'POST',
      url: 'http://127.0.0.1:8000/api/matchRecipe/',
      contentType: 'application/json',
      data: JSON.stringify({ 'listCategories': list_recipe }),
